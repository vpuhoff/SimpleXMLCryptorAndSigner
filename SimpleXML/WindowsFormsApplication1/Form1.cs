...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        private void Button3_Click(object sender, EventArgs e)
        {
            ...
            Button3.Visible  = false;//отключаем кнопку "подписать"
            rsaKey = GetKey();
            signProg.Maximum = filestosign.Length; //устанавливаем максимальное значение для Ambiance.Ambiance_ProgressBar 
            MethodInvoker ThreadRun = new MethodInvoker(delegate //запускаем процесс в отдельном потоке, чтобы не тормозить форму
                    {
                        try
                        {
                            ...
                            foreach (var file in filestosign)
                            {
                                ...
                                var xmlDoc = CreateXMLFromAnyFile(file );
                                xmlDoc.PreserveWhitespace = true;
                                SignXml(xmlDoc, rsaKey);
                                string filename = signsdir + Path.GetFileNameWithoutExtension(file) + "_signed" + Path.GetExtension(file)+".xml";
                                xmlDoc.Save(filename);
                                n++;
                                signProg.Invoke(new Action(() => signProg.Value = n)); //обновляем прогресс бар в потоке формы
                            }
                            Button3.Invoke(new Action(() => Button3.Visible = true));//включаем кнопку обратно
        ...
        private void Button5_Click(object sender, EventArgs e)
        {
            ...
            var xmlfilescount = Directory.GetFiles(DocsFolderSigned.Text , "*.xml").Length;
            if (xmlfilescount > 0)
            {
                rsaKey = GetKey();
            }
            string checkeddir = DocsFolderChecked.Text + "\\";
            if (!Directory.Exists(checkeddir))
            {
                Directory.CreateDirectory(checkeddir);
            }
            var files = Directory.GetFiles(DocsFolderSigned.Text,"*.xml");
            if (files.Length ==0)
            {
                ...
                try
                {
                    n=0;
                     foreach (var file in files)
                     {
                         ...
                         sg.FileName = Path.GetFileName(file);
                         if (result == true)
                         {
                             var sf = GetSF(file);
                             var data = GetDataFromBase64(sf.Binary);
                             sg.FileName = sf.FileName;
                             File.WriteAllBytes(checkeddir + sf.FileName,data );
                             sg.Result = "OK";
                         }
                         else
                         {
                    ...
                    Button5.Invoke(new Action(() => Button5.Visible = true));//включаем кнопку обратно
        ...
        string GetKeyHash(RSA key)
        {
            return GetBase64(key.EncryptValue(UTF8Encoding.UTF8.GetBytes("TestKey")));
        }
        public XmlDocument CreateXMLFromAnyFile(string filepath)
        {
            SignedFile sf = new SignedFile();
            sf.Binary = GetBase64(File.ReadAllBytes(filepath ));
            sf.FileName = Path.GetFileName(filepath);
            XmlDocument xmldoc = new XmlDocument() ;
            string xmltext;
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                {
                    XmlSerializer xmlWriter = new XmlSerializer(sf.GetType());
                    xmlWriter.Serialize(sw, sf);
                    sw.Flush();
                    ms.Position = 0;
        ...
        SignedFile GetSF(string file){
            ...
            using (MemoryStream ms = new MemoryStream(data))
            {
                ...
                {
                    var xmlSerializer = new XmlSerializer(typeof(SignedFile));
                    sf = (SignedFile)xmlSerializer.Deserialize(sr);
                }
            }
            return sf;
        }
